...
(function () {
  ...
  function embedRows(embeds) {
    return embeds.slice().sort((a, b) => String(a.mark).localeCompare(String(b.mark), undefined, {
      ...
      'SC Delivery': stubDeliveryOf(e),
      'SC Received': e.hasStub ? e.stubDeliveredAt || '' : '',
  ...
  function printDeliveryList(rows, meta) {
    ...
    const totals = rows.reduce((a, r) => ({
      marks: a.marks + 1,
      bTot: a.bTot + N(r.bTot),
      bDel: a.bDel + N(r.bDel),
      sTot: a.sTot + N(r.sTot),
      sDel: a.sDel + N(r.sDel),
    ...
    const body = rows.map(r => {
      const st = STATUS[r.state] || STATUS.none;
