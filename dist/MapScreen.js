...
function pourTitle(z, n) {
  return z && z.name ? z.name : '#' + (n || '?');
}
...
function isPoly(z) {
  return Array.isArray(z && z.points) && z.points.length >= 3;
}
function rectToPoly(z) {
  return [[z.x, z.y], [z.x + z.w, z.y], [z.x + z.w, z.y + z.h], [z.x, z.y + z.h]];
}
function zonePts(z) {
  return isPoly(z) ? z.points : rectToPoly(z);
}
function bboxOf(pts) {
  let x0 = 1e9,
    y0 = 1e9,
...
function setGridLine(cfg, axis, i, nf) {
  ...
  const arr = isCol ? cfg.cols : cfg.rows;
  const wkey = isCol ? 'colW' : 'rowH';
  const n = arr.length,
    pl = cfg.plan,
    lo = isCol ? 'x0' : 'y0',
    hi = isCol ? 'x1' : 'y1';
  ...
  if (i >= n - 1) return {
    ...
    plan: {
      ...pl,
      [hi]: clampN(nf, pl[lo] + 0.02, 1)
    }
  };
  const f = cumFrac(cfg[wkey], n);
  const pf = (nf - pl[lo]) / (pl[hi] - pl[lo] || 1);
  f[i] = clampN(pf, f[i - 1] + 0.004, f[i + 1] - 0.004);
  const w = [];
  for (let j = 0; j < n - 1; j++) w.push(Math.max(0.004, f[j + 1] - f[j]));
  return {
    ...cfg,
    [wkey]: w
  };
...
function MapScreen({
  ...
  const [scLayer, setScLayer] = React.useState(() => {
    try {
      return localStorage.getItem('embedyap_sclayer') !== '0';
    } catch (_) {
      return true;
    }
  });
  React.useEffect(() => {
    try {
      localStorage.setItem('embedyap_sclayer', scLayer ? '1' : '0');
    } catch (_) {}
  }, [scLayer]);
  ...
  const plan = React.useMemo(() => {
    ...
    const aw = box.w - pad * 2,
      ah = box.h - pad * 2;
    let pw = Math.min(aw, ah * BP_W / BP_H);
    let ph = pw * BP_H / BP_W;
    return {
      pw,
      ph,
  ...
  function focusZone(z) {
    ...
    const cx = (bb.x + bb.w / 2) * plan.pw,
      cy = (bb.y + bb.h / 2) * plan.ph; // pan/zoom to a pour
    const s = clampS(2.2);
  ...
  React.useEffect(() => {
    if (drawMode !== 'pin') setGhost(null);
  }, [drawMode]); // clear cursor preview
  ...
  React.useEffect(() => {
    ...
    const onWheelNative = e => {
      ...
      touched.current = true;
      setView(v => {
        const ns = clampS(v.s * (e.deltaY < 0 ? 1.12 : 0.89));
        const k = ns / v.s;
        return {
          ...v,
          s: ns,
  ...
  function mapDelivery(e) {
    return scLayer ? siteDeliveryState(e) : deliveryState(e);
  }
  ...
  function drawPin(ctx, e, x, y, on, picked) {
    ...
    if (colorMode === 'delivery') {
      col = DELIVERY[mapDelivery(e)].color;
  ...
  function onPointerDown(e) {
    if (e.button === 1) {
      ...
      try {
        vpRef.current.setPointerCapture(e.pointerId);
      } catch (err) {} // middle-click drag = pan, any tool
      touched.current = true;
      drag.current = {
        x: r.x,
        y: r.y,
    ...
    if (drawMode === 'pin') {
      const key = onAddPin({
        x: +f.fx.toFixed(4),
        y: +f.fy.toFixed(4),
        embedId: place.mark || 'NEW',
        knifePlate: place.knifePlate,
        stubColumn: place.stubColumn,
    ...
    if (drawMode === 'poly') {
      setPoly(p => [...p, [+f.fx.toFixed(4), +f.fy.toFixed(4)]]);
      return;
    }
  ...
  function onPointerMove(e) {
    ...
    if (gdrag.current) {
      ...
      onGridDraft(setGridLine(gridDraft, g.axis, g.i, g.axis === 'col' ? f.fx : f.fy));
      return;
    }
    if (drawMode === 'pin') {
      setGhost({
    ...
    if (vdrag.current) {
      setLiveZones(applyVertex(vdrag.current.id, vdrag.current.vi, f.fx, f.fy));
      return;
    }
    if (sdrag.current) {
      setLiveZones(translateZone(sdrag.current, f.fx - sdrag.current.fx, f.fy - sdrag.current.fy));
      return;
    }
    if (drawing.current) {
      drawing.current = {
    ...
    if (marquee.current) {
      ...
      setMarq(normZone(marquee.current));
      return;
    }
    if (drag.current) {
      setView(v => ({
  ...
  function onPointerUp(e) {
    ...
    if (pdrag.current) {
      ...
      setDragPins(null);
      if (pd.moved && pd.last) {
        const prev = pd.ids.filter(id => pd.base[id]).map(id => ({
          id,
          x: pd.base[id].nx,
          y: pd.base[id].ny
        }));
        const next = pd.ids.filter(id => pd.last[id]).map(id => ({
          id,
          x: +pd.last[id].nx.toFixed(4),
          y: +pd.last[id].ny.toFixed(4)
        }));
        pushUndo({
          type: 'pinMove',
          moves: prev
    ...
    if (vdrag.current) {
      commitLive(vdrag.current.id);
      vdrag.current = null;
      return;
    }
    if (sdrag.current) {
      commitLive(sdrag.current.id);
      sdrag.current = null;
      return;
    }
    if (drawing.current) {
      const z = normZone(drawing.current);
  ...
  function workingZone(id) {
    return (liveZones || zones).find(z => z.id === id) || zones.find(z => z.id === id);
  }
  function applyVertex(id, vi, fx, fy) {
    const z = workingZone(id);
    if (!z) return liveZones;
    const pts = zonePts(z).map((p, i) => i === vi ? [+fx.toFixed(4), +fy.toFixed(4)] : p);
    return mergeLive({
      ...z,
      points: pts,
      ...bboxOf(pts)
  ...
  function translateZone(sd, dx, dy) {
    const z = zones.find(zz => zz.id === sd.id);
    if (!z) return liveZones;
    const pts = zonePts(sd.base).map(([x, y]) => [+(x + dx).toFixed(4), +(y + dy).toFixed(4)]);
    return mergeLive(isPoly(sd.base) ? {
      ...z,
      points: pts,
      ...bboxOf(pts)
    } : {
      ...z,
      x: +(sd.base.x + dx).toFixed(4),
      y: +(sd.base.y + dy).toFixed(4)
    });
  }
  function mergeLive(merged) {
    return (liveZones || zones).map(z => z.id === merged.id ? merged : z);
  }
  function commitLive(id) {
    const z = (liveZones || []).find(zz => zz.id === id);
    setLiveZones(null);
    if (!z) return;
    const prev = zones.find(zz => zz.id === id);
    const patch = isPoly(z) ? {
      points: z.points,
      x: z.x,
      y: z.y,
      w: z.w,
  ...
  function addVertex(id, edgeIdx) {
    const z = zones.find(zz => zz.id === id);
    if (!z) return;
    const pts = zonePts(z);
    const a = pts[edgeIdx],
      b = pts[(edgeIdx + 1) % pts.length];
    const mid = [+((a[0] + b[0]) / 2).toFixed(4), +((a[1] + b[1]) / 2).toFixed(4)];
    const np = [...pts.slice(0, edgeIdx + 1), mid, ...pts.slice(edgeIdx + 1)];
    pushUndo({
      type: 'set',
      id,
      z: {
  ...
  function removeVertex(id, vi) {
    const z = zones.find(zz => zz.id === id);
    if (!z) return;
    const pts = zonePts(z);
    if (pts.length <= 3) return;
    const np = pts.filter((_, i) => i !== vi);
    pushUndo({
      type: 'set',
      id,
      z: {
  ...
  function pushUndo(inv) {
    undo.current.push(inv);
    if (undo.current.length > 80) undo.current.shift();
  }
  function doUndo() {
    const inv = undo.current.pop();
    if (!inv) return;
    if (inv.type === 'remove') onRemoveZone(inv.id);else if (inv.type === 'set') {
      inv.z ? onRestoreZone(inv.id, inv.z) : onRemoveZone(inv.id);
    } else if (inv.type === 'pinRemove') onRemovePin(inv.id);else if (inv.type === 'pinRemoveMany') (inv.ids || []).forEach(id => onRemovePin(id));else if (inv.type === 'pinRestore') onRestorePin(inv.id, inv.data);else if (inv.type === 'pinMove') onMovePins(inv.moves);
  }
  function deleteZone(id) {
    const prev = zones.find(z => z.id === id);
    if (!prev) return;
    pushUndo({
      type: 'set',
      id,
  ...
  function pasteZone() {
    ...
    const z = clip.current;
    const off = 0.025;
    const payload = isPoly(z) ? {
      ...z,
      points: z.points.map(([x, y]) => [+(x + off).toFixed(4), +(y + off).toFixed(4)]),
      x: z.x + off,
      y: z.y + off
    } : {
      ...z,
  ...
  function copyEmbeds() {
    if (selPins.length > 0) {
      const items = selPins.map(id => embeds.find(x => x.id === id)).filter(Boolean).map(e => ({
        mark: e.mark,
        knifePlate: !!e.hasKnife,
      ...
      const e = embeds.find(x => x.id === selId);
      if (e) clipEmbed.current = {
        multi: false,
        items: [{
          mark: e.mark,
          knifePlate: !!e.hasKnife,
    ...
    if (c.multi) {
      ...
      c.items.forEach(it => {
        const key = onAddPin({
          ...
          y: (it.ny || 0) + off,
          embedId: it.mark || 'NEW',
          knifePlate: it.knifePlate,
          stubColumn: it.stubColumn,
  ...
  function requestDeletePins(ids) {
    if (!ids.length) return;
    if (ids.length > 1) setConfirm({
      message: `Delete ${ids.length} embeds? You can undo with ⌘Z.`,
      onYes: () => {
        deletePins(ids);
        setConfirm(null);
      }
  ...
  React.useEffect(() => {
    function onKey(e) {
      ...
      else if (meta && /^[aA]$/.test(e.key)) {
        e.preventDefault();
        setSelPins(visible.map(em => em.id));
        setSelId(null);
      } else if (meta && /^[cC]$/.test(e.key)) {
        if (selPins.length || selId) {
          e.preventDefault();
          copyEmbeds();
        } else if (manager && selZone) {
          e.preventDefault();
          copyZone(selZone);
        }
      } else if (meta && /^[vV]$/.test(e.key)) {
        if (clipEmbed.current) {
          e.preventDefault();
          pasteEmbeds();
        } else if (manager) {
          e.preventDefault();
  ...
  const visible = embeds.filter(e => (filter === 'all' || pinState(e) === filter) && (cat === 'all' || (cat === 'kp' ? e.hasKnife : !e.hasKnife)) && (stub === 'all' || (stub === 'sc' ? e.hasStub : !e.hasStub)) && (!ql || String(e.mark || '').toLowerCase().includes(ql) || String(e.grid || '').toLowerCase().includes(ql) || e.hasStub && String(e.stubType || '').toLowerCase().includes(ql)));
  const counts = STATE_ORDER.reduce((m, k) => {
    m[k] = embeds.filter(e => pinState(e) === k).length;
    return m;
  }, {});
  const deliveryCounts = DELIVERY_ORDER.reduce((m, k) => {
    m[k] = embeds.filter(e => mapDelivery(e) === k).length;
    return m;
  }, {}); // matches the painted pin colors, SC layer included
  ...
  function clearNextPour() {
    ...
    zones.filter(z => z.nextPour).forEach(z => onUpdateZone(z.id, {
      nextPour: false
    }));
  }
  function commitZone(z) {
    ...
    if (z.attachTo) {
      pushUndo({
        type: 'set',
        id: z.attachTo,
        z: zones.find(zz => zz.id === z.attachTo)
      });
      onUpdateZone(z.attachTo, {
        ...bb,
        ...(z.points ? {
          points: z.points
        } : {
    ...
    if (z._new) {
      ...
      pushUndo({
        type: 'set',
        id: z.id,
        z: zones.find(zz => zz.id === z.id)
      });
      onUpdateZone(z.id, payload);
    }
    setEditZone(null);
  }
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, /*#__PURE__*/React.createElement(MapToolbar, {
    filter,
    setFilter,
    cat,
    setCat,
    ...
    onToggleGrid: () => {
      const next = !gridMode;
      setGridMode(next);
      onGridDraft(next ? buildGridCfg(savedGrid) : null);
    },
    manager,
    isPhone,
    pourMode,
    ...
    zoomIn: () => zoomAt(box.w / 2, box.h / 2, 1.25),
    zoomOut: () => zoomAt(box.w / 2, box.h / 2, 0.8),
    reset: fit,
    scale: view.s
    ...
    style: {
      ...
      left: view.tx,
      top: view.ty,
      width: plan.pw * view.s,
      height: plan.ph * view.s,
      borderRadius: 8,
      boxShadow: '0 0 0 1px rgba(150,170,205,.18), 0 30px 90px -30px rgba(0,0,0,.9)'
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'absolute',
  ...
  }, /*#__PURE__*/React.createElement(Blueprint, {
    grid: grid
  })), gridMode && gridDraft && /*#__PURE__*/React.createElement(React.Fragment, null, gridDraft.cols.map((c, i) => {
    const gx = colX(i);
    const edge = i === 0 || i === gridDraft.cols.length - 1;
    return /*#__PURE__*/React.createElement("div", {
      key: 'gch' + i,
      onPointerDown: e => {
        e.stopPropagation();
      ...
      style: {
        width: edge ? 3 : 2,
        height: '100%',
        background: edge ? 'rgba(245,194,75,.9)' : 'rgba(82,230,224,.7)'
      }
    }), /*#__PURE__*/React.createElement("div", {
      style: {
        position: 'absolute',
        top: 2,
        background: edge ? '#F5C24B' : '#52E6E0',
        color: '#06140e',
  ...
  }), gridDraft.rows.map((r, j) => {
    const gy = rowY(j);
    const edge = j === 0 || j === gridDraft.rows.length - 1;
    return /*#__PURE__*/React.createElement("div", {
      key: 'grh' + j,
      onPointerDown: e => {
        e.stopPropagation();
    ...
    return /*#__PURE__*/React.createElement("g", {
      ...
      "data-zone": true,
      points: dpts,
      fill: `rgba(${gc},${z.done ? 0.20 : 0.12})`,
      stroke: `rgba(${gc},${on ? 1 : 0.9})`,
      strokeWidth: on ? swU * 1.6 : swU,
  ...
  }), poly.length > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("polyline", {
    points: poly.map(p => p.join(',')).join(' '),
    fill: "rgba(126,120,240,.12)",
    stroke: T.color.amberHot,
    strokeWidth: swU
  }), poly.map((p, i) => /*#__PURE__*/React.createElement("circle", {
    key: 'pp' + i,
    cx: p[0],
    cy: p[1],
    r: handleU * 0.8,
    fill: T.color.amberHot
  }))), draft && (() => {
    const z = normZone(draft);
    return /*#__PURE__*/React.createElement("rect", {
      x: z.x,
      y: z.y,
      width: z.w,
      height: z.h,
      fill: "rgba(126,120,240,.14)",
      stroke: T.color.amberHot,
      strokeWidth: swU,
      strokeDasharray: `${swU * 3} ${swU * 3}`
    });
  })(), marq && /*#__PURE__*/React.createElement("rect", {
    x: marq.x,
    y: marq.y,
    width: marq.w,
    height: marq.h,
    fill: "rgba(166,160,255,.12)",
    stroke: T.color.amberHot,
    strokeWidth: swU,
    ...
    if (isPour) {
      ...
      }, z.nextPour ? 'NEXT' : '', z.nextPour && z.date ? ' · ' : '', z.date ? shortDate(z.date) : ''));
    }
    const lbl = `${seqLabel(z.pour)} · ${z.area}${z.done ? ' ✓' : ''}`;
    return /*#__PURE__*/React.createElement("div", {
      key: 'lb' + z.id,
      style: {
        position: 'absolute',
    ...
    style: {
      ...
      height: 13,
      borderRadius: '50%',
      background: T.color.amberHot,
      border: '2px dashed #fff',
      boxShadow: '0 0 10px rgba(166,160,255,.7)'
    }
  }, place.knifePlate && /*#__PURE__*/React.createElement("span", {
    style: {
      position: 'absolute',
      top: -5,
      left: -5,
  ...
  }), place.stubColumn && /*#__PURE__*/React.createElement("span", {
    style: {
      position: 'absolute',
      bottom: -5,
      left: -5,
    ...
    plan: plan,
    box: box
  }), drawMode === 'pin' && /*#__PURE__*/React.createElement("div", {
    "data-ui": true,
    style: {
      position: 'absolute',
      top: 14,
  ...
  }, "Placing embeds")), /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 12.5,
      color: T.color.steel300,
      margin: 0,
  ...
  }, "Click anywhere on the plan to drop an anchor bolt \u2014 it keeps adding so you can place several, then hit Done."), /*#__PURE__*/React.createElement(Field, {
    label: "Mark / piece"
  }, /*#__PURE__*/React.createElement("input", {
    value: place.mark,
    onChange: ev => setPlace(p => ({
      ...p,
    ...
    style: {
      ...
      textTransform: 'uppercase',
      letterSpacing: '.03em',
      color: place.knifePlate ? T.color.blue : '#fff'
    }
  }, "Knife plate"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11.5,
      color: T.color.steel300,
      marginTop: 2
    }
  }, "Is there a knife plate here?")), /*#__PURE__*/React.createElement(Toggle, {
    on: place.knifePlate,
    onChange: () => setPlace(p => ({
      ...p,
    ...
    style: {
      ...
      textTransform: 'uppercase',
      letterSpacing: '.03em',
      color: place.stubColumn ? '#FF9650' : '#fff'
    }
  }, "Stub column"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11.5,
      color: T.color.steel300,
      marginTop: 2
    }
  }, "Is there a stub column here?")), /*#__PURE__*/React.createElement(Toggle, {
    on: place.stubColumn,
    onChange: () => setPlace(p => ({
      ...p,
  ...
  }, "Done")), selPins.length > 0 && drawMode === 'off' && /*#__PURE__*/React.createElement("div", {
    "data-ui": true,
    style: {
      position: 'absolute',
      top: 14,
    ...
    kind: "primary",
    icon: "check",
    onClick: () => onBulkInstall(selPins, true)
  }, "Mark done"), /*#__PURE__*/React.createElement(Btn, {
    size: "sm",
    kind: "ghost",
    icon: "close",
    ...
    icon: "layers",
    onClick: () => copyEmbeds()
  }, "Copy"), manager && /*#__PURE__*/React.createElement(Btn, {
    size: "sm",
    kind: "danger",
    icon: "trash",
    onClick: () => requestDeletePins(selPins)
  }, "Delete"), /*#__PURE__*/React.createElement(Btn, {
    size: "sm",
    kind: "ghost",
    icon: "close",
    onClick: () => setSelPins([])
  }, "Clear")), selectedZone && manager && !editZone && selPins.length === 0 && /*#__PURE__*/React.createElement("div", {
    "data-ui": true,
    style: {
      position: 'absolute',
      top: 14,
  ...
  }, reshape ? 'Reshaping' : 'Reshape'), /*#__PURE__*/React.createElement(Btn, {
    size: "sm",
    kind: "ghost",
    icon: "layers",
    onClick: () => copyZone(selectedZone.id)
    ...
    onDelete: editZone._new ? null : () => {
      deleteZone(editZone.id);
      setEditZone(null);
    }
  }), gridMode && manager && gridDraft && /*#__PURE__*/React.createElement(GridEditor, {
    cfg: gridDraft,
    onChange: onGridDraft,
    onClose: () => {
      setGridMode(false);
      onGridDraft(null);
    },
    onSave: () => {
      onSaveGrid(gridDraft);
      setGridMode(false);
    },
    onReset: () => {
      onSaveGrid(null);
  ...
  }), showPours && /*#__PURE__*/React.createElement(PoursDrawer, {
    zones: zones,
    embeds: embeds,
    manager: manager,
    user: user,
...
function MapToolbar({
  ...
  return /*#__PURE__*/React.createElement("div", {
    style: {
      ...
      background: steelPlate('#141A24', '#0E131B'),
      position: 'relative',
      zIndex: 6
    }
  }, /*#__PURE__*/React.createElement("div", {
    ...
    name: pourMode ? 'clipboard' : 'target',
    size: 18,
    style: {
      color: T.color.amber
    }
  ...
  }, pourMode ? 'Pour map' : 'Plan')), /*#__PURE__*/React.createElement("div", {
    style: {
      width: 1,
      height: 24,
      background: T.color.line
    ...
    style: {
      ...
      padding: '0 11px',
      borderRadius: T.radius.md,
      background: fOpen || activeFilters ? 'rgba(126,120,240,.18)' : 'rgba(0,0,0,.3)',
      border: '1px solid ' + (fOpen || activeFilters ? 'rgba(126,120,240,.6)' : T.color.line),
      color: fOpen || activeFilters ? '#A6A0FF' : T.color.steel200,
      fontFamily: T.font.display,
      fontWeight: 700,
      fontSize: 12,
      letterSpacing: '.03em'
    ...
    name: "filter",
    size: 14
  }), "Filters", activeFilters ? /*#__PURE__*/React.createElement("span", {
    style: {
      background: '#A6A0FF',
      color: '#0A0B16',
      borderRadius: 8,
  ...
  }, activeFilters) : null), fOpen && /*#__PURE__*/React.createElement("div", {
    "data-ui": true,
    style: {
      position: 'absolute',
      top: 36,
  ...
  }, /*#__PURE__*/React.createElement(Kicker, null, "Filters"), activeFilters > 0 && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setFilter('all');
      setCat('all');
      setStub('all');
  ...
  }, "RESET")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement(Kicker, null, "Show"), /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 5
    }
  }, /*#__PURE__*/React.createElement(Segmented, {
  ...
  }))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement(Kicker, null, "Knife plate"), /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 5
    }
  }, /*#__PURE__*/React.createElement(Segmented, {
  ...
  }))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement(Kicker, null, "Stub column"), /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 5
    }
  }, /*#__PURE__*/React.createElement(Segmented, {
  ...
  })), colorMode === 'delivery' && /*#__PURE__*/React.createElement("button", {
    onClick: () => setScLayer(v => !v),
    title: scLayer ? 'Stub columns counted — a pin stays red until its stub column is on site. Click for bolts only.' : 'Bolts only — pin color ignores stub columns. Click to count them again.',
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 7,
      ...
      border: '1px solid ' + (scLayer ? 'rgba(255,150,80,.5)' : T.color.line),
      borderRadius: T.radius.md
    }
  }, /*#__PURE__*/React.createElement("span", {
    style: {
      ...
      flex: '0 0 auto',
      background: scLayer ? '#FF9650' : 'transparent',
      border: '1.5px solid ' + (scLayer ? '#FF9650' : T.color.steel400)
    }
  }), /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: T.font.mono,
      fontSize: 11,
      letterSpacing: '.04em',
      color: scLayer ? '#FF9650' : T.color.steel300
    }
  }, scLayer ? 'SC ON' : 'BOLTS ONLY')), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 7,
    ...
    placeholder: "Find mark / stub type\u2026",
    style: {
  ...
  }, manager && drawMode === 'poly' && poly.length >= 3 && /*#__PURE__*/React.createElement(Btn, {
    size: "sm",
    kind: "primary",
    icon: "check",
    onClick: finishPoly
  }, "Finish"), manager && (drawMode === 'rect' || drawMode === 'poly') && /*#__PURE__*/React.createElement(Btn, {
    size: "sm",
    kind: "ghost",
    icon: "close",
    onClick: cancel
  }, "Cancel"), drawMode === 'off' && !pourMode && /*#__PURE__*/React.createElement(Btn, {
    size: "sm",
    kind: "ghost",
    icon: "pinAdd",
    onClick: () => setDrawMode('pin')
  }, "Add embed"), manager && drawMode === 'off' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Btn, {
    size: "sm",
    kind: "ghost",
    icon: "zone",
    onClick: () => setDrawMode('rect')
  }, "Box"), /*#__PURE__*/React.createElement(Btn, {
    size: "sm",
    kind: "ghost",
    icon: "polygon",
    onClick: () => setDrawMode('poly')
  }, "Polygon"), /*#__PURE__*/React.createElement(Btn, {
    size: "sm",
    kind: gridMode ? 'primary' : 'ghost',
    icon: "grid",
    ...
    return /*#__PURE__*/React.createElement("button", {
      key: L,
      onClick: () => setLayerVis(v => ({
        ...v,
        [L]: !v[L]
      })),
      title: (on ? 'Hide ' : 'Show ') + L + ' markups',
      style: {
        display: 'flex',
        alignItems: 'center',
        gap: 5,
        ...
        padding: '0 9px',
        borderRadius: T.radius.md,
        background: on ? `rgba(${rgb},.18)` : 'rgba(0,0,0,.3)',
        border: '1px solid ' + (on ? `rgba(${rgb},.6)` : T.color.line),
        color: on ? `rgb(${rgb})` : T.color.steel400,
        fontFamily: T.font.display,
        fontWeight: 700,
        fontSize: 11,
        letterSpacing: '.04em'
    ...
    icon: "clipboard",
    onClick: () => setPours(p => !p)
  }, "Pours"), /*#__PURE__*/React.createElement("button", {
    onClick: toggleFull,
...
function Legend({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, /*#__PURE__*/React.createElement(Kicker, null, delivery ? boltsOnly ? 'Delivery · bolts' : 'Delivery' : 'Legend'), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    style: {
      color: T.color.steel400
    }
  ...
  }, STATE[k].label), /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: T.font.mono,
      fontSize: 12,
      color: T.color.steel300
  ...
  }, "Knife plate"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: T.font.mono,
      fontSize: 12,
      color: T.color.steel300
    }
  }, knifeCount)), !boltsOnly && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 9,
  ...
  }, "Stub column"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: T.font.mono,
      fontSize: 12,
      color: T.color.steel300
    }
  }, stubCount)), boltsOnly && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 9,
    ...
    style: {
      width: 9,
      height: 9,
      border: '1.5px solid ' + T.color.steel400,
      display: 'inline-block',
      marginLeft: 1
    }
  }), /*#__PURE__*/React.createElement("span", {
  ...
  }, stubCount, " SC hidden")), delivery && !boltsOnly && stubCount > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 9,
  ...
  }, /*#__PURE__*/React.createElement("span", null, "TOTAL PINNED"), /*#__PURE__*/React.createElement("span", {
    style: {
      color: '#fff'
    }
  }, total)));
...
function GridEditor({
  ...
  const cols = cfg.cols || [],
    ...
    rowH = cfg.rowH || [];
  function addCol() {
    const last = cols[cols.length - 1] || 'A';
    const next = /^[A-Z]$/.test(last) ? String.fromCharCode(Math.min(90, last.charCodeAt(0) + 1)) : last + "'";
    set({
      cols: [...cols, next],
      colW: [...colW, colW[colW.length - 1] || 1]
    });
  }
  function delCol() {
  ...
  function addRow() {
    ...
    const next = isNaN(n) ? last + "'" : String(n - 1);
    set({
      rows: [...rows, next],
      rowH: [...rowH, rowH[rowH.length - 1] || 1]
    });
  }
  function delRow() {
  ...
  const bay = (key, i) => e => {
    const v = [...cfg[key]];
    v[i] = Math.max(0.05, +e.target.value || 0.05);
    set({
      [key]: v
    });
  };
  const margin = k => e => set({
    plan: {
      ...cfg.plan,
      [k]: Math.max(0, Math.min(1, +e.target.value || 0))
    }
  });
  function evenBays(key, n) {
    set({
      [key]: Array(Math.max(0, n - 1)).fill(1)
    });
  }
  const fld = {
    ...inputStyle,
  ...
  const tabBtn = (k, label) => /*#__PURE__*/React.createElement("button", {
    key: k,
    onClick: () => setTab(k),
    style: {
      flex: 1,
      ...
      fontSize: 11.5,
      letterSpacing: '.04em',
      background: tab === k ? 'rgba(126,120,240,.2)' : 'rgba(0,0,0,.25)',
      border: '1px solid ' + (tab === k ? 'rgba(126,120,240,.6)' : T.color.line),
      color: tab === k ? '#A6A0FF' : T.color.steel400
    }
  }, label);
  return /*#__PURE__*/React.createElement("div", {
    "data-ui": true,
    style: {
      ...
      right: 0,
      bottom: 0,
      width: isPhoneNow() ? '100%' : 344,
      zIndex: 20,
      display: 'flex',
      flexDirection: 'column',
      background: steelPlate('#141B26', '#0E131B'),
      borderLeft: '1px solid ' + T.color.line,
      boxShadow: '-20px 0 60px -30px rgba(0,0,0,.9)'
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
  ...
  }, "Edit grid")), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    style: {
      color: T.color.steel400
    }
  ...
  }, tabBtn('cols', 'Columns'), tabBtn('rows', 'Rows'), tabBtn('plan', 'Margins')), /*#__PURE__*/React.createElement("div", {
    style: {
    ...
    name: "target",
    size: 12
  }), " Drag the lines on the map to align them to the embeds."), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1,
      overflowY: 'auto',
      padding: '4px 16px 16px'
    }
  }, tab === 'plan' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: sec
  }, "Plan margins (0\u20131) \u2014 outer grid box"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
      gridTemplateColumns: '1fr 1fr',
      gap: 8
    }
  }, [['x0', 'Left'], ['x1', 'Right'], ['y0', 'Top'], ['y1', 'Bottom']].map(([k, l]) => /*#__PURE__*/React.createElement("label", {
    key: k,
    style: {
      display: 'flex',
      flexDirection: 'column',
  ...
  }, "The outer (amber) lines are the plan box \u2014 drag them on the map or set them here.")), tab === 'cols' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      ...sec,
      display: 'flex',
      justifyContent: 'space-between',
  ...
  }, /*#__PURE__*/React.createElement("span", null, "Columns (", cols.length, ")"), /*#__PURE__*/React.createElement("span", {
    style: {
      display: 'flex',
      gap: 6
    }
    ...
    title: "Even spacing",
    onClick: () => evenBays('colW', cols.length),
    style: miniBtn
  }, /*#__PURE__*/React.createElement(Icon, {
    name: "grid",
    size: 12
  ...
  }), i < cols.length - 1 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: T.font.mono,
      fontSize: 11,
      color: T.color.steel400
    ...
    title: "Bay width to next column"
  }))))), tab === 'rows' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      ...sec,
      display: 'flex',
      justifyContent: 'space-between',
  ...
  }, /*#__PURE__*/React.createElement("span", null, "Rows (", rows.length, ")"), /*#__PURE__*/React.createElement("span", {
    style: {
      display: 'flex',
      gap: 6
    }
    ...
    title: "Even spacing",
    onClick: () => evenBays('rowH', rows.length),
    style: miniBtn
  }, /*#__PURE__*/React.createElement(Icon, {
    name: "grid",
    size: 12
  ...
  }), i < rows.length - 1 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: T.font.mono,
      fontSize: 11,
      color: T.color.steel400
...
function GridRuler({
  ...
  for (let i = 0; i < C.length; i++) {
    const x = view.tx + colX(i) * plan.pw * view.s;
    if (x >= 26 && x <= box.w - 6) cols.push([x, C[i]]);
  }
  for (let i = 0; i < R.length; i++) {
    const y = view.ty + rowY(i) * plan.ph * view.s;
    if (y >= 22 && y <= box.h - 6) rows.push([y, R[i]]);
  }
  const chip = {
    position: 'absolute',
    fontFamily: T.font.mono,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      left: 0,
      width: 26,
      background: 'linear-gradient(90deg,rgba(8,11,16,.55),transparent)'
    }
  }), cols.map(([x, l], k) => /*#__PURE__*/React.createElement(React.Fragment, {
    key: 'c' + k
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'absolute',
  ...
  }, l))), rows.map(([y, l], k) => /*#__PURE__*/React.createElement(React.Fragment, {
    key: 'r' + k
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'absolute',
...
function poursItems(z) {
  const stored = Array.isArray(z.checklist) ? z.checklist : [];
  const have = new Set(stored.map(i => i && i.text));
  const merged = stored.slice();
  (window.PREPOUR_DEFAULT || []).forEach(t => {
    if (!have.has(t)) merged.push({
      text: t,
      done: false
    });
  });
...
function inspDueInfo(z, doneN, total) {
  const due = inspDueDate(z && z.date);
  if (!due) return null;
  const dleft = inspDaysUntil(due);
  const complete = total > 0 && doneN === total;
  let col = T.color.steel400,
    lbl = 'Due ' + shortDate(due);
  if (complete) col = T.color.green;else if (dleft < 0) {
    col = T.color.red;
    lbl = 'Overdue · ' + shortDate(due);
  } else if (dleft === 0) {
    col = T.color.yellow;
    lbl = 'Due today';
  } else if (dleft === 1) {
...
function PoursDrawer({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Pours \xB7 pre-pour")), /*#__PURE__*/React.createElement("button", {
    onClick: onClose,
    style: {
      color: T.color.steel400
    }
    ...
    name: "close",
    size: 16
  }))), manager && anyNext && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '9px 16px',
      borderBottom: '1px solid ' + T.color.line,
      display: 'flex',
  ...
  }, "Next-pour tags active"), /*#__PURE__*/React.createElement(Btn, {
    size: "sm",
    kind: "ghost",
    icon: "close",
    onClick: onClearNextPour
  }, "Clear next-pour")), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1,
      overflowY: 'auto',
      padding: '10px 12px 16px'
    }
  }, pours.length === 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: T.font.mono,
      fontSize: 12.5,
      color: T.color.steel400,
  ...
  }, "No pours yet. Draw a ", /*#__PURE__*/React.createElement("b", {
    style: {
      color: '#fff'
    }
  }, "Box"), " or ", /*#__PURE__*/React.createElement("b", {
    style: {
      color: '#fff'
    }
  }, "Polygon"), " on the map and set its layer to ", /*#__PURE__*/React.createElement("b", {
    style: {
      color: '#fff'
    }
  }, "WCG Pours"), " \u2014 it shows here with a pre-pour checklist."), pours.map((z, i) => /*#__PURE__*/React.createElement(PourRow, {
    key: z.id,
    z: z,
    n: i + 1,
    manager: manager,
...
const POUR_STATUS_RGB = {
  PENDING: '59,130,246',
  ORDERED: '245,158,11',
  COMPLETE: '47,214,166'
}; // matches CUP dashboard
function pourStatus(z) {
  return z.status || (z.done ? 'COMPLETE' : 'PENDING');
}
function PourRow({
  z,
  n,
  ...
  function toggle(i) {
    const it = items[i];
    writeItems(items.map((x, k) => k === i ? it.done ? {
      ...x,
      done: false,
      by: null,
      at: null
  ...
  function rename(i, text) {
    writeItems(items.map((x, k) => k === i ? {
      ...x,
      text
    } : x));
  }
  function remove(i) {
    writeItems(items.filter((_, k) => k !== i));
  }
  function add() {
    writeItems([...items, {
      text: 'New item',
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, sub)), dueInfo && /*#__PURE__*/React.createElement("span", {
    title: "Inspections due (day before pour)",
    style: {
      display: 'inline-flex',
      alignItems: 'center',
      gap: 3,
  ...
  }), dueInfo.lbl), placed ? /*#__PURE__*/React.createElement("button", {
    onClick: () => onGoto && onGoto(z),
    title: "Go to pour on map",
    style: {
      color: T.color.cyan,
      padding: 4
    }
    ...
    title: "Draw its box on the map to place it",
    style: {
      ...
      fontWeight: 700,
      padding: '2px 5px',
      border: '1px solid rgba(245,194,75,.4)',
      borderRadius: 5
    }
    ...
    style: {
      fontFamily: T.font.mono,
      fontSize: 12,
      color: doneN === items.length && items.length ? T.color.green : T.color.steel300
    }
  }, doneN, "/", items.length), /*#__PURE__*/React.createElement("button", {
    onClick: () => setExp(e => !e),
    style: {
      color: T.color.steel400,
      padding: 2
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      style: {
        ...
        fontSize: 10.5,
        letterSpacing: '.04em',
        background: on ? `rgba(${rgb},.18)` : 'rgba(0,0,0,.25)',
        border: '1px solid ' + (on ? `rgba(${rgb},.6)` : T.color.line),
        color: on ? `rgb(${rgb})` : T.color.steel400
      }
    }, s);
  ...
  }), manager ? /*#__PURE__*/React.createElement("input", {
    type: "time",
    defaultValue: z.time || '',
    onBlur: e => {
      if (e.target.value !== (z.time || '')) onUpdateZone(z.id, {
        time: e.target.value
      });
    },
    style: {
  ...
  }, z.time ? fmtTimeEmbed(z.time) : '—')), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 6,
  ...
  }, +z.cy || 0), /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: T.font.mono,
      fontSize: 10,
      color: T.color.steel400
    }
  }, "CY"))), mix.psi || mix.slump || mix.aggregate || mix.lc3 ? /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexWrap: 'wrap',
      gap: '4px 12px',
  ...
  }, mix.psi ? /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("span", {
    style: {
      color: T.color.steel400
    }
  }, "PSI "), mix.psi) : null, mix.slump ? /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("span", {
    style: {
      color: T.color.steel400
    }
  }, "Slump "), mix.slump) : null, mix.aggregate ? /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("span", {
    style: {
      color: T.color.steel400
    }
  }, "Agg "), mix.aggregate) : null, mix.lc3 ? /*#__PURE__*/React.createElement("span", {
    style: {
      color: T.color.green
    }
  }, "LC3 \u2713") : null) : null, /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'space-between',
  ...
  }, "Pre-pour ", doneN, "/", items.length), dueInfo && /*#__PURE__*/React.createElement("span", {
    style: {
      display: 'inline-flex',
      alignItems: 'center',
      gap: 4,
  ...
  }), dueInfo.lbl)), items.map((it, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    style: {
      display: 'flex',
      alignItems: 'center',
    ...
    style: {
      ...
      display: 'grid',
      placeItems: 'center',
      background: it.done ? T.color.green : 'rgba(0,0,0,.25)',
      border: '1px solid ' + (it.done ? T.color.green : T.color.line)
    }
    ...
    onBlur: e => {
      if (e.target.value !== it.text) rename(i, e.target.value);
    },
    style: {
      ...inputStyle,
      flex: 1,
    ...
    style: {
      flex: 1,
      fontSize: 13,
      color: it.done ? T.color.steel300 : '#fff',
      textDecoration: it.done ? 'line-through' : 'none'
    }
  }, it.text), it.done && it.by && /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: T.font.mono,
      fontSize: 9.5,
      color: T.color.steel400,
  ...
  }, it.by.split(' ')[0], it.at ? ' · ' + shortDate(it.at) : ''), manager && /*#__PURE__*/React.createElement("button", {
    onClick: () => remove(i),
    title: "Remove",
    style: {
      color: T.color.steel400,
    ...
    defaultValue: z.cupPourId || '',
    placeholder: "CUP pour (e.g. B-2)",
    onBlur: e => {
      if (e.target.value !== (z.cupPourId || '')) onUpdateZone(z.id, {
        cupPourId: e.target.value
      });
    },
    style: {
      ...inputStyle,
    ...
    kind: "primary",
    icon: "arrowRight",
    onClick: () => window.open(CUP_LIVE_URL, '_blank', 'noopener')
  }, "Go to live pour"))));
}
