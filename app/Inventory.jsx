...
function Inventory({ embeds, isPhone, types, onEditType, onAddType, onDeleteType, onSyncQtys, onBulkDelivery, onBulkInstall, canEdit, userName, seqMeta={}, onSetSeqNeeded, wcgPours=[] }){
  ...
  function printDelivery(){
    ...
    out.forEach(e=>{ const s = seqMode==='wcg' ? (e.wcgPour||'—') : (e.sequence||'—');
      (byS[s] || (byS[s] = {})); const m = byS[s][e.mark] || (byS[s][e.mark] = { mark:e.mark, desc:e.typeLabel, none:0, transit:0, grids:[], areas:new Set(), knife:0, stub:0 });
      if(dState(e)==='transit') m.transit++; else m.none++;
      if(e.grid) m.grids.push(e.grid); if(e.area) m.areas.add(e.area);
      if(e.hasKnife) m.knife++; if(e.hasStub && window.stubDeliveryState(e)!=='delivered') m.stub++; });
    ...
    const groups = Object.keys(byS).map(s=>{
      ...
      const rowsOut = Object.values(byS[s]).map(m=>({
        ...
        qty: m.none + m.transit, none: m.none, transit: m.transit,
      ...
      })).sort((a,b)=> (b.none-a.none) || String(a.mark).localeCompare(String(b.mark), undefined, {numeric:true}));
