  ...
  function printDelivery(){
    ...
    const flat = Object.values(byMarkFlat).sort((a,b)=> num(a.mark, b.mark)).map(m=>{
      ...
      return { mark:m.mark, desc:m.desc,
        ...
        state: outstanding===0 ? 'on-site'
             : (m.bTot>0 && m.bDel===m.bTot && m.sDel<m.sTot) ? 'stub-short'
             : (m.bDel+m.sDel)>0 ? 'partial' : 'none',
      ...
      <div className="ey-fade" style={{ maxWidth:1080, margin:'0 auto', padding:isPhone?'18px 14px 90px':'28px 30px 60px' }}>
        <Header title="Inventory" sub={`By embed type · ${rows.length} marks${seqFilter!=='all'?' · '+(seqMode==='wcg'?'WCG ':'')+seqLabelOf(seqFilter):''}${delivSubLabel}`}>
          ...
          <Btn kind="ghost" size="sm" icon="inventory" onClick={printDelivery}
            title="Print an 11x17 quick-reference of every mark in numerical order — bolts and stub columns delivered vs outstanding. Switch Delivered off above to print a pull list instead.">Print 11×17</Btn>
