 ...
 */
const DELIV_STATES = [
  { key:'none',      label:'Not delivered', color:'#F4674F' },   // not on site / not ordered
  { key:'transit',   label:'On the way',    color:'#F5C24B' },   // shipped, not landed
  { key:'delivered', label:'Delivered',     color:'#3FBF7F' },   // on site
];
...
function Inventory({ embeds, isPhone, types, onEditType, onAddType, onDeleteType, onSyncQtys, onBulkDelivery, onBulkInstall, canEdit, userName, seqMeta={}, onSetSeqNeeded, wcgPours=[] }){
  ...
  const [delivShow, setDelivShow] = React.useState({ none:true, transit:true, delivered:true });
  const toggleDeliv = (k)=> setDelivShow(s=>{ const n={ ...s, [k]:!s[k] };
  ...
  function printDelivery(){
    ...
    const byS = {};
    out.forEach(e=>{ const s = seqMode==='wcg' ? (e.wcgPour||'—') : (e.sequence||'—');
      (byS[s] || (byS[s] = {}));
      const m = byS[s][e.mark] || (byS[s][e.mark] = { mark:e.mark, desc:e.typeLabel,
        bNone:0, bTransit:0, sNone:0, sTransit:0, grids:[], areas:new Set(), knife:0, stubTypes:new Set() });
      const b = dState(e);
      ...
      if(e.grid) m.grids.push(e.grid); if(e.area) m.areas.add(e.area);
    ...
    const groups = Object.keys(byS).map(s=>{
      ...
      const rowsOut = Object.values(byS[s]).map(m=>({
        mark: m.mark, desc: m.desc,
        bolts: m.bNone + m.bTransit, bNone: m.bNone, bTransit: m.bTransit,
        stubs: m.sNone + m.sTransit, sNone: m.sNone, sTransit: m.sTransit,
      ...
      })).sort((a,b)=> ((b.bNone+b.sNone)-(a.bNone+a.sNone)) || String(a.mark).localeCompare(String(b.mark), undefined, {numeric:true}));
      return { key:s, label: seqLabelOf(s), needed, daysLeft: ni?ni.days:null, rows: rowsOut };
    })
  ...
  return (
    <div style={{ position:'absolute', inset:0, overflowY:'auto' }}>
      <div className="ey-fade" style={{ maxWidth:1080, margin:'0 auto', padding:isPhone?'18px 14px 90px':'28px 30px 60px' }}>
        <Header title="Inventory" sub={`By embed type · ${rows.length} marks${seqFilter!=='all'?' · '+(seqMode==='wcg'?'WCG ':'')+seqLabelOf(seqFilter):''}${delivSubLabel}`}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6, background:'rgba(0,0,0,.3)', border:'1px solid '+(!delivAll?'rgba(245,194,75,.55)':T.color.line),
            borderRadius:T.radius.md, padding:'0 8px', height:32 }} title="Show or hide each delivery status — they are independent, so Not delivered + On the way gives you everything still to come">
            <Icon name="inventory" size={14} style={{ color:!delivAll?'#F5C24B':T.color.steel400 }} />
            {DELIV_STATES.map(s=>{ const on=!!delivShow[s.key]; return (
              <span key={s.key} onClick={()=>toggleDeliv(s.key)} title={(on?'Hide ':'Show ')+s.label}
                style={{ display:'inline-flex', alignItems:'center', gap:5, cursor:'pointer', userSelect:'none',
                  padding:'3px 8px', borderRadius:T.radius.sm, whiteSpace:'nowrap',
                  background:on?'rgba(255,255,255,.07)':'transparent',
                  border:'1px solid '+(on?s.color+'88':'transparent'),
                  color:on?'#fff':T.color.steel500, fontSize:11.5,
                  textDecoration:on?'none':'line-through', opacity:on?1:.6 }}>
                <span style={{ width:7, height:7, borderRadius:'50%', background:on?s.color:T.color.steel500, flexShrink:0 }} />
                {s.label}
              </span>); })}
          </div>
          ...
          <Btn kind="ghost" size="sm" icon="inventory" onClick={printDelivery}
            title="Print an 11x17 sheet of every anchor bolt not yet on site — grouped by sequence, with tick boxes for the yard">Delivery list 11×17</Btn>
